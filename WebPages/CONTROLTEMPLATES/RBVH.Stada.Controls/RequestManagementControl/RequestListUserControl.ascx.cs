    ...
    public partial class RequestListUserControl : ListBaseUserUserControl
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            try
            {
                ...
                this.dtcFromDate.SelectedDate = DateTime.Now;
                this.dtcToDate.SelectedDate = DateTime.Now;
        ...
        protected override void LoadListOfDepartment()
        {
            try
            {
                if (this.ddlDepartments != null)
                {
                    ...
                    departments.Insert(0, new Department { ID = Department_All, Name = "All", VietnameseName = "Tất cả" });
        ...
        {
            return base.GetFromDateString(BizConstants.TypeOfDateRange.FromTo);
        }
        ...
        {
            return base.GetToDateString(BizConstants.TypeOfDateRange.FromTo);
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                this.gridRequestsByReceivedDepartment.AllowPaging = true;
                this.gridRequestsByReceivedDepartment.PageSize = BizConstants.GridViewSettings.PageSize;
        ...
        private void GridRequestsByReceivedDepartment_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                if (e.Row.RowType == System.Web.UI.WebControls.DataControlRowType.DataRow)
                {
                    ...
                    if (dataItem.Department != null && dataItem.Department.LookupId > 0)
                    {
                        departmentName = GetDepartmentName(dataItem.Department.LookupId);
                    }
        ...
        private void LoadFormSearchRequestsByReceivedDepartment()
        {
            try
            {
                if (IsVietnameseLanguage)
                {
                    ...
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.InProcess, Text = VietnameseApprovalStatus.InProcess });
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.Completed, Text = VietnameseApprovalStatus.Completed });
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.Rejected, Text = VietnameseApprovalStatus.Rejected });
                }
                else
                {
                    //var listStatus = ApprovalStatusMapping;
                    ...
                    ddlStatus.Items.Insert(0, emptyListItem);
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.InProcess, Text = ApprovalStatus.InProcess });
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.Completed, Text = ApprovalStatus.Completed });
                    ddlStatus.Items.Add(new ListItem { Value = ApprovalStatus.Rejected, Text = ApprovalStatus.Rejected });
                }
            }
            catch (Exception ex)
            {
        ...
        private Tuple<string, string> BuildDateRange()
        {
