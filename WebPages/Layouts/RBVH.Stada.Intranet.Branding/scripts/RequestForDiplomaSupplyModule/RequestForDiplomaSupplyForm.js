...
RBVH.Stada.WebPages.pages.RequestForDiplomaSupplyForm.prototype =
    {
        ...
        AppendInputFile: function () {
            ...
            $(that.Settings.Controls.DocumentFilesDivSelector).append("<input type='file' name='supportingDocument" + countNumer + "' style='display:inline'>  <span id='supportingDocument" + countNumer + "' class='glyphicon glyphicon-trash span-remove'></span>  <span spanErrorName='supportingDocument" + countNumer + "' class='ms-formvalidation ms-csrformvalidation'></span><br/>")
        },
        // Validation
        ...
        IsFormValueChanged: function () {
            ...
            changedCount += ($(that.Settings.SupportingDocuments).not(supportingDocumentDeletedItems).length === 0 && $(supportingDocumentDeletedItems).not(that.Settings.SupportingDocuments).length === 0) ? 0 : 1;
