...
namespace RBVH.Stada.Intranet.WebPages.CONTROLTEMPLATES.RBVH.Stada.Controls.Common
{
    ...
    {
        /// <summary>
        /// Get hidden field deleted items
        /// </summary>
        public HiddenField DeletedItems
        {
            get
            {
                return this.hfDeletedItems;
            }
        }
        ...
        private string BuildLinkToItem(string webUrl, SPListItem listItem)
        {
            ...
            try
            {
                ...
                linkToItem = string.Format("<a href='{0}' target='_blank'>{1}</a><span name='" + fileName + "' id='supportingDocument_" + listItem.ID + "' class='glyphicon glyphicon-trash span-delete supporting-item-deleted'></span></br/>", urlOfFile, listItem.Name);
            }
            catch(Exception ex)
            {
                ULSLogging.LogError(ex);
