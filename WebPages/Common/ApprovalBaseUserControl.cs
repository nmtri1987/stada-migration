...
namespace RBVH.Stada.Intranet.WebPages.Common
{
    ...
    public class ApprovalBaseUserControl : FormBaseUserControl
    {
        ...
        /// </summary>
        protected SupportingDocumentControl supportingDocumentControlObject;
        ...
        public SupportingDocumentControl SupportingDocumentControlObject
        {
            get
            {
                return this.supportingDocumentControlObject;
            }
        }
        ...
        protected override bool SaveForm()
        {
            ...
            if (res)
            {
                ...
                this.DeleteSupportingDocuments();
        ...
        private void InitSupportingDocumentControl()
        {
            var listOfSupportingDocumentControls = this.GetAllControlsOfType<SupportingDocumentControl>().ToList();
            if (listOfSupportingDocumentControls != null && listOfSupportingDocumentControls.Count > 0)
            {
                this.supportingDocumentControlObject = listOfSupportingDocumentControls[0];
            }
        }
        ...
        protected virtual void Inits()
        {
            ...
            // Init supporting document control
            InitSupportingDocumentControl();
        ...
        /// </summary>
        /// <returns></returns>
        private bool DeleteSupportingDocuments()
        {
            var res = true;

            try
            {
                if (this.supportingDocumentControlObject != null)
                {
                    var deletedItemsValue = supportingDocumentControlObject.DeletedItems.Value;
                    if (!string.IsNullOrEmpty(deletedItemsValue))
                    {
                        JavaScriptSerializer seriallizer = new JavaScriptSerializer();
                        var deletedItems = seriallizer.Deserialize<List<string>>(deletedItemsValue);
                                ...
                                using (var site = new SPSite(this.CurrentWeb.Url))
                                {
                                    using (SPWeb web = site.OpenWeb())
                                    {
                                        ...
                                        foreach(var deletedItem in deletedItems)
                                        {
                                            try
                                            {
                                                var file = web.GetFile(deletedItem);
                                                file.Delete();
                                            }
                                            catch (Exception ex)
                                            {
